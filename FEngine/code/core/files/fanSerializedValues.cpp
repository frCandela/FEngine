...
namespace fan {
	...
	void SerializedValues::GetVec2	( const char * _name, btVector2&		 _outVec2 ) { ISerializable::LoadVec2( m_json[m_valuesName], _name, _outVec2 ); }
	void SerializedValues::GetVec3	( const char * _name, btVector3&		 _outVec3 ) { ISerializable::LoadVec3( m_json[m_valuesName], _name, _outVec3 ); }
	void SerializedValues::GetQuat	( const char * _name, btQuaternion&		 _outQuat ) { ISerializable::LoadQuat( m_json[m_valuesName], _name, _outQuat ); }
	void SerializedValues::GetColor	( const char * _name, Color&			 _outColor ) { ISerializable::LoadColor( m_json[m_valuesName], _name, _outColor ); }
	void SerializedValues::GetFloat	( const char * _name, float&			 _outFloat ) { ISerializable::LoadFloat( m_json[m_valuesName], _name, _outFloat ); }
	void SerializedValues::GetInt	( const char * _name, int&				 _outInt ) { ISerializable::LoadInt( m_json[m_valuesName], _name, _outInt ); }
	void SerializedValues::GetUInt	( const char * _name, unsigned&			 _outUInt ) { ISerializable::LoadUInt( m_json[m_valuesName], _name, _outUInt ); }
	void SerializedValues::GetBool	( const char * _name, bool&				 _outBool ) { ISerializable::LoadBool( m_json[m_valuesName], _name, _outBool ); }
	void SerializedValues::GetString( const char * _name, std::string&		 _outString ) { ISerializable::LoadString( m_json[m_valuesName], _name, _outString ); }
}