...
namespace fan
{
	namespace scene
	{
		...
		void FPSCamera::Update(const float _delta) {
			...
			fan::Engine::GetEngine().GetRenderer().DebugLine(offset, offset + btVector3(size, 0, 0), Color(1, 0, 0, 1));
			fan::Engine::GetEngine().GetRenderer().DebugLine(offset, offset + btVector3(0, size, 0), Color(0, 1, 0, 1));
			fan::Engine::GetEngine().GetRenderer().DebugLine(offset, offset + btVector3(0, 0, size), Color(0, 0, 1, 1));
		}
