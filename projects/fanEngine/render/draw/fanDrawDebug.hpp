...
namespace fan
{
    ...
    struct DrawDebug
    {
        ...
        Pipeline           mPipelineTrianglesNDT;
        ...
        std::vector<Buffer> mVertexBuffersTrianglesNDT;
        std::vector<Buffer> mVertexBuffersLines2D;
        int                 mNumLines        = 0;
        int                 mNumLinesNDT     = 0;
        int                 mNumTriangles    = 0;
        int                 mNumTrianglesNDT = 0;
        int                 mNumLines2D      = 0;

        void Create( Device& _device, uint32_t _imagesCount );
        void Destroy( Device& _device );
        void UpdateUniformBuffers( Device& _device, const size_t _index );
        bool HasNothingToDraw() const { return mNumLines == 0 && mNumLinesNDT == 0 && mNumTriangles == 0 && mNumTrianglesNDT == 0 && mNumLines2D == 0; }
        ...
        void SetDebugDrawData( const uint32_t _index, Device& _device, const std::vector<DebugLineVertex>& _lines, const std::vector<DebugLineVertex>& _linesNoDepthTest,
        ...
        PipelineConfig GetPipelineConfigTrianglesNDT() const;
