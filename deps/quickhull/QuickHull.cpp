...
namespace quickhull {
	...
	MeshBuilder<T> QuickHull<T>::getInitialTetrahedron() {
		...
		if (maxD == m_epsilon) {
			...
			m_planar = true;
			const vec3 N2 = mathutils::getTriangleNormal(baseTriangleVertices[1],baseTriangleVertices[2],baseTriangleVertices[0]);
			m_planarPointCloudTemp.clear();
			m_planarPointCloudTemp.insert(m_planarPointCloudTemp.begin(),m_vertexData.begin(),m_vertexData.end());
			const vec3 extraPoint = N2 + m_vertexData[0];
		...
		for (auto& f : mesh.m_faces) {
			...
			const Vector3<T> N3 = mathutils::getTriangleNormal(va, vb, vc);
			const Plane<T> trianglePlane2(N3,va);
			f.m_P = trianglePlane2;
		}
