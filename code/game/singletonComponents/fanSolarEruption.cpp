...
namespace fan
{
	...
	//================================================================================================================================
	//================================================================================================================================
	std::string SolarEruption::StateToString( const State  _state )
	{
		switch( _state )
		{
		case WAITING:		return "waiting";		 break;
		case WARMING:		return "warming";		 break;
		case COLLAPSING:	return "collapsing";	 break;
		case EXPODING:		return "exploding";		 break;
		case BACK_TO_NORMAL:return "back to normal"; break;
	...
	//================================================================================================================================
	//================================================================================================================================
	void SolarEruption::OnGui( SingletonComponent& _component )
	{
		SolarEruption& solarEruption = static_cast<SolarEruption&>( _component );
		...
		ImGui::Indent(); ImGui::Indent();
		{
			ImGui::PushItemWidth( 200.f );
			{
				...
				ImGui::Text( "next eruption: %f", solarEruption.stateDuration[WAITING] );
	...
	void SolarEruption::Start( EcsWorld& _world )
	{
		...
		eruption.stateDuration[WAITING] = eruption.cooldown + Random::Float() * eruption.randomCooldown;
	...
	void SolarEruption::Step( EcsWorld& _world, const float _delta )
	{
		...
		if( eruption.timer > eruption.stateDuration[eruption.state] )
		{
			...
			eruption.state = State( ( eruption.state + 1 ) % State::SIZE );
			//eruption.state = EXPODING;
			eruption.timer = 0.f;
