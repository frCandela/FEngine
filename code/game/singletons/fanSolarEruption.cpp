...
namespace fan
{
	...
	void SolarEruption::Step( EcsWorld& _world, const float _delta )
	{
		...
		case BACK_TO_NORMAL:
		{
			...
			eruption.sunlightLight->attenuation[PointLight::LINEAR] =
			    eruption.previousLightAttenuation + delta * ( target - eruption.previousLightAttenuation );
		}break;
		case EXPODING:
		{
			const float target = eruption.stateLightAttenuation[eruption.state];
	...
	void SolarEruption::Save( const EcsSingleton& _component, Json& _json )
	{
		...
		for( int i = 0; i < SIZE; i++ )
		{
            ...
            Serializable::SaveFloat( _json,
                                     ...
                                     solarEruption.stateLightAttenuation[i] );
            Serializable::SaveFloat( _json,
                                     ( name + "_particle_per_second" ).c_str(),
                                     solarEruption.stateParticlePerSecond[i] );
            Serializable::SaveFloat( _json,
                                     ( name + "_particle_speed" ).c_str(),
                                     solarEruption.stateParticleSpeed[i] );
            Serializable::SaveFloat( _json,
                                     ( name + "_particle_duration" ).c_str(),
                                     solarEruption.stateParticleDuration[i] );
            Serializable::SaveColor( _json,
                                     ( name + "_particle_color" ).c_str(),
                                     solarEruption.stateParticleColor[i] );
		}
	}
	...
	void SolarEruption::OnGui( EcsWorld& _world, EcsSingleton& _component )
	{
		...
		ImGui::PushItemWidth( 200.f );
		{
            ...
            const float nextEruptionTime = time.logicDelta *
                                           float ( solarEruption.spawnFrame - time.frameIndex );
			...
			for( int i = 0; i < SIZE; i++ )
			{
				...
				if( ImGui::CollapsingHeader( ( "state: " + name ).c_str() ) )
				{
                    ...
                    ImGui::ColorEdit4( ( "state particle color##" + name ).c_str(),
                                       solarEruption.stateParticleColor[i].Data(),
